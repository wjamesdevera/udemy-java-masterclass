/**
 * We have a dog that likes to bark.  We need to wake up if the dog is barking
 * at night!
 *
 ...
 * name hourOfDay and has a valid range of 0-23.
 *
...
public class Main {
    ...
    public static boolean shouldWakeUp(boolean barking, int hourOfDay) {
        boolean isValidHourOfDay = (hourOfDay >= 0 && hourOfDay <= 23);
        if (!isValidHourOfDay) {
            return false;
        }

        boolean shouldWakeUp = barking && (hourOfDay < 8 || hourOfDay > 22);
        return shouldWakeUp;
    }
}